...
{
    // define a list to hole CMGCell objects
    private List<CMGCell> cellList;

    // constructor to initialize the cellList
    public CMGReader()
    {
        cellList = new List<CMGCell>();
    }

    // method to add a CMGCell to the list
    //public void Add(int screenX, int screenY, int width, int height, byte[] image, byte[] mask)
    //{
    //    // create a new CMGCell object with the provided parameters
    ...
    //        Width = (uint)width,
    //        Height = (uint)height,
    //        Images = new byte[][] { image }, // wrap the image in a byte array and assign it to images
    //        Masks = new byte[][] { mask } // wrap the mask in a byte array and assign it to masks
    //    };

    //    // add the newCell to the cellList
    ...
    //public static void SaveToFile(CMGCell[] cells, string filePath)
    //{
    //    {
    //        // use CMGWriter to write the cells to specified file path
